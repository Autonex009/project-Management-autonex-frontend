...
const accentTheme = {
 pm: {
 active: 'bg-white text-slate-900 font-semibold ring-1 ring-slate-200/60 shadow-[0_1px_2px_rgba(16,24,40,0.04)] ',
 inactive: 'text-slate-600 font-medium hover:text-slate-900 hover:bg-white/70 ',
...
const iconBtn = 'w-9 h-9 flex items-center justify-center rounded-md border border-slate-200 text-slate-500 hover:text-slate-900 hover:bg-slate-100 transition-colors';
...
const EmployeeSidebar = ({ user = {}, account, role, isPm, onNavigate, onLogout }) => {
 ...
 <img src="/favicon.png" alt="Autonex" className="h-9 w-9 rounded-lg border border-slate-200 bg-white p-1 shrink-0" />
 <div className="min-w-0">
 <p className="text-[15px] font-semibold text-slate-900 truncate leading-tight">Autonex</p>
 <p className="text-[12px] text-slate-400 truncate leading-tight">{portalLabel}</p>
 </div>
 </div>
 </div>
 ...
 <button type="button" className="h-9 w-9 flex items-center justify-center overflow-hidden rounded-lg border border-slate-200 bg-white ">
 {avatarUrl ? (
