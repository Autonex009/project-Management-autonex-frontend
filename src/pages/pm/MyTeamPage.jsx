...
const MyTeamPage = () => {
  ...
  const teamMembers = employees
    ...
    .map((employee) => {
      ...
      const totalDailyHours = memberProjects.reduce(
  ...
  const roleOptions = useMemo(
    () =>
      Array.from(
        new Set(
          teamMembers.map((member) => member.designation).filter(Boolean),
        ),
  ...
  const filteredMembers = useMemo(() => {
    ...
    return teamMembers.filter((member) => {
      const matchesName =
        !term || (member.name || "").toLowerCase().includes(term);
      const matchesProject =
        projectFilter === "all" ||
        member.memberProjects.some(
          (project) => String(project.id) === projectFilter,
        );
      const matchesRole =
        roleFilter === "all" || member.designation === roleFilter;
      return matchesName && matchesProject && matchesRole;
    });
  }, [teamMembers, searchTerm, projectFilter, roleFilter]);

  const hasActiveFilters =
    searchTerm.trim() !== "" || projectFilter !== "all" || roleFilter !== "all";
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        ...
        <StatCard
          ...
          value={teamMembers.reduce(
            (sum, member) => sum + member.totalDailyHours,
            0,
          )}
          unit="h"
          icon={Clock3}
      ...
      {isLoading ? (
        ...
        <div className="rounded-3xl border border-dashed border-slate-200 bg-white p-12 text-center shadow-sm">
          ...
          <p className="mt-2 text-sm text-slate-500">
            Team members will appear here once employees are allocated to
            projects inside your PM scope.
          </p>
        </div>
            ...
            >
              <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                <div className="flex items-start gap-4">
                  {/* Profile picture, falling back to initials when the employee
                      has no avatar (or the URL 404s — UserAvatar handles onError). */}
                  <UserAvatar
              ...
              <div className="mt-5 grid gap-3 md:grid-cols-2">
                ...
                <InfoRow
                  icon={Phone}
                  label="Phone"
                  value={member.phone || "Not available"}
                />
                <InfoRow
                  icon={Briefcase}
                  label="Employee Type"
                  value={member.employee_type || "Not available"}
                />
                <InfoRow
                  icon={Users}
                  label="Skills"
...
const SlackDmButton = ({ employee }) => {
  ...
  const handleClick = async () => {
    ...
    try {
      ...
      setError(
        status === 404 ? "Slack account not found" : "Slack unavailable",
      );
    } finally {
      setLoading(false);
    }
