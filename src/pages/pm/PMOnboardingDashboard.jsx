...
const PMOnboardingDashboard = ({ embedded = false }) => {
  ...
  useEffect(() => {
    ...
    onboardingApi
      ...
      .catch(async (err) => {
        console.warn(
          "getMentees API failed, trying fallback from reports:",
          err,
        );
        try {
          ...
          setMentees(
            filtered.map((c) => ({
              ...
              isActive: true,
              completedModulesCount: (c.moduleStats || []).filter(
                (m) => m.progress === 100,
              ).length,
    ...
    return (
      <div className="flex items-center justify-center min-h-[400px] text-slate-500 font-medium">
        <Spinner
          size="md"
          color="indigo"
          text="Loading your mentorship dashboard..."
        />
      </div>
    );
  }
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
          <div>
            ...
            <p className="text-slate-500 text-sm mt-1">
              Monitor training progression and quiz outcomes for your team.
            </p>
          </div>
          ...
          {mentees.length === 0 ? (
            <div className="p-12 text-center flex flex-col items-center max-w-md mx-auto">
              ...
              <p className="text-sm text-slate-400 text-center mt-1">
                When administrator links employees to your supervision, their
                module checklist status and quiz averages will display here.
              </p>
            </div>
          ) : (
            <Table
              ...
              columns={[
                ...
                },
                {
                  ...
                  render: (value) => (
                    <span
                      className={`text-[9px] uppercase font-bold tracking-wider px-2 py-0.5 rounded-full ${value ? "bg-emerald-50 text-emerald-700 border border-emerald-100" : "bg-slate-100 text-slate-400 border border-slate-200"}`}
                    >
