...
import { ChevronDown, ChevronUp, ClipboardList, FolderKanban, Clock } from 'lucide-react';
...
const PerformanceReviewsPage = () => {
 ...
 const scopedIds = useMemo(() => new Set(scopedProjects.map((p) => p.id)), [scopedProjects]);
 const submissionsCount = evaluations.filter((e) => scopedIds.has(e.project_id)).length;
 const pendingCount = evaluations.filter((e) => e.status === 'submitted' && scopedIds.has(e.project_id)).length;
 ...
 {/* Header */}
 <div>
 <h1 className="text-lg font-semibold text-slate-900">Monthly Performance Reviews</h1>
 ...
 <StatCard title="Projects" value={scopedProjects.length} icon={FolderKanban} tone="indigo" hint="in your scope" />
 <StatCard title="Submissions" value={submissionsCount} icon={ClipboardList} tone="sky" hint="employee reviews" />
 <StatCard title="Pending Review" value={pendingCount} icon={Clock} tone="amber" hint="awaiting your approval" />
 </div>
