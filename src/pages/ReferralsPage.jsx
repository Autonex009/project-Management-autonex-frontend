...
const ReferralsPage = () => {
  ...
  const statusMutation = useMutation({
    mutationFn: ({ id, status, note }) =>
      referralApi.updateStatus(id, status, note),
    onSuccess: () => {
  ...
  const filtered = referrals.filter((r) => {
    ...
    return (
      candidateName.includes(q) || position.includes(q) || referrer.includes(q)
    );
  });
  ...
  const stats = {
    ...
    active: referrals.filter((r) =>
      ["reviewing", "interview_scheduled"].includes(r.status),
    ).length,
    ...
    <div className="space-y-4">
          ...
          {TABS.map((tab) => {
            ...
            return (
              ...
              >
                ...
                {count > 0 && tab !== "All" && (
                  <span
                    className={`inline-flex items-center justify-center h-4 min-w-[16px] rounded-full px-1 text-[10px] font-bold ${active ? "bg-indigo-100 text-indigo-700" : "bg-slate-200 text-slate-600"}`}
                  >
                    {count}
                  </span>
                )}
                ...
                >
                  <div className="px-5 py-3.5 flex items-center justify-between gap-4">
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      <Button size="sm" onClick={() => openStatusModal(ref)}>
                        Update Status
                      </Button>
                      <button
                        onClick={() =>
      ...
      {statusModal && (
        <Modal isOpen onClose={() => setStatusModal(null)} size="md">
          ...
          <Modal.Footer>
            <Button variant="cancel" onClick={() => setStatusModal(null)}>
              Cancel
            </Button>
            <Button
              onClick={() =>
