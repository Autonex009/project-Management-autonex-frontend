...
const SignupRequestsPage = () => {
 ...
 useEffect(() => {
  ...
  const updateMutation = useMutation({
    ...
    onError: (err) =>
      toast.error(
        err.response?.data?.detail || "Failed to update employment type",
      ),
  });

  const rejectMutation = useMutation({
    mutationFn: ({ id, reason }) =>
      signupRequestApi.reject(id, user.id, reason),
    onSuccess: () => {
      queryClient.invalidateQueries(["signup-requests"]);
      queryClient.invalidateQueries(["signup-requests-counts"]);
      setRejectModal(null);
  ...
  const undoApproveMutation = useMutation({
    mutationFn: ({ id, reason }) =>
      signupRequestApi.undoApprove(id, user.id, reason),
    onSuccess: () => {
      queryClient.invalidateQueries(["signup-requests"]);
      queryClient.invalidateQueries(["signup-requests-counts"]);
      setRejectModal(null);
    ...
    <div className="space-y-4">
      ...
      <Table
        ...
        emptyState={{
          ...
          description: "Applicant signup requests will appear here.",
        }}
                ...
                <div className="flex flex-wrap gap-1.5">
                  {(req.skills || []).length > 0 ? (
                    req.skills.map((s) => (
                      <span
                        key={s}
                        className="px-2 py-0.5 rounded-md bg-indigo-50 text-indigo-700 border border-indigo-200 text-[12px] font-medium"
                      >
                        {s}
                      </span>
                    ))
                ...
                <div className="flex items-center justify-end gap-2">
                  {req.status === "rejected" && (
                    <Button
                      variant="secondary"
                      size="sm"
                      onClick={() => undoRejectMutation.mutate(req.id)}
      ...
      {rejectModal && (
        <Modal isOpen onClose={() => setRejectModal(null)} size="md">
          ...
          <Modal.Footer>
            <Button variant="cancel" onClick={() => setRejectModal(null)}>
              Cancel
            </Button>
            <Button
              variant="danger"
