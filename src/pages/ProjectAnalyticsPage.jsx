...
import { Clock, Users, Gauge, ChevronLeft, MessageSquare, ChevronDown, CheckSquare, Square } from 'lucide-react';
...
const ProjectAnalyticsPage = () => {
 ...
 useEffect(() => {
 const handler = (e) => { if (annotatorRef.current && !annotatorRef.current.contains(e.target)) setAnnotatorOpen(false); };
 ...
 const allVisibleOn = visibleEmails.length > 0 && visibleEmails.every((e) => activeSel.includes(e));
 const selectAllVisible = () => setSelected([...new Set([...activeSel, ...visibleEmails])]);
 const deselectAllVisible = () => setSelected(activeSel.filter((e) => !visibleEmails.includes(e)));

 // Switching group tab also switches the chart selection to that group's members.
 const changeGroup = (key) => {
 setOrgFilter(key);
 const emails = key === 'all'
 ? allEmails
 ...
 className={`px-3 py-1.5 text-[13px] font-semibold rounded-md transition-all whitespace-nowrap ${rangeMode === r.key
 ...
 className={`inline-flex items-center gap-1.5 px-3 py-1.5 text-[13px] font-semibold rounded-md transition-all whitespace-nowrap ${effectiveFilter === t.key
 ? 'bg-white text-slate-900 shadow-sm ring-1 ring-slate-200/70'
 : 'text-slate-500 hover:text-slate-800'
 }`}
 >
 {t.label}
 <span className={effectiveFilter === t.key ? 'text-indigo-600' : 'text-slate-400'}>{t.count}</span>
 </button>
 ))}
 </div>
 ...
 Annotators
 <span className="inline-flex items-center justify-center h-4 min-w-4 px-1 rounded-full bg-indigo-600 text-white text-[10px] font-semibold">{activeSel.length}</span>
 <ChevronDown className={`w-4 h-4 text-slate-400 transition-transform ${annotatorOpen ? 'rotate-180' : ''}`} />
 </button>
 {annotatorOpen && (
 ...
 {orgTabs.find((t) => t.key === effectiveFilter)?.label || 'All'} · {visibleAnnotators.length}
 </span>
 <button
 type="button"
 onClick={allVisibleOn ? deselectAllVisible : selectAllVisible}
 className="inline-flex items-center gap-1 text-xs font-medium text-indigo-600 hover:text-indigo-700"
 >
 {allVisibleOn ? <Square className="w-3.5 h-3.5" /> : <CheckSquare className="w-3.5 h-3.5" />}
 {allVisibleOn ? 'Deselect all' : 'Select all'}
 </button>
 </div>
 {visibleAnnotators.length === 0 ? (
 <p className="py-3 text-sm text-slate-400">No annotators in this group.</p>
 ) : (
 ...
 >
 {on ? <CheckSquare className="w-4 h-4 shrink-0 text-indigo-600" /> : <Square className="w-4 h-4 shrink-0 text-slate-300" />}
 <span className="h-2 w-2 shrink-0 rounded-full" style={{ backgroundColor: colorFor(a.user_email) }} />
 <span className="min-w-0 flex-1 truncate text-slate-700">{a.employee_name || a.user_email}</span>
 <span className="shrink-0 tabular-nums text-slate-400">{a.total_hours}h</span>
 </button>
 );
 })}
 </div>
 ...
 <div className={`flex flex-wrap gap-x-4 gap-y-1.5 ${legendExpanded ? '' : 'max-h-6 overflow-hidden'}`}>
 {activeSel.map((email) => {
 const a = annotators.find((x) => x.user_email === email);
 return (
 <span key={email} className="inline-flex items-center gap-1.5 whitespace-nowrap text-xs text-slate-600">
 <span className="h-2.5 w-2.5 shrink-0 rounded-full" style={{ backgroundColor: colorFor(email) }} />
 ...
 >
 {legendExpanded ? 'Show less' : `Show all ${activeSel.length}`}
 <ChevronDown className={`h-3.5 w-3.5 transition-transform ${legendExpanded ? 'rotate-180' : ''}`} />
 </button>
 )}
 </div>
 )}
