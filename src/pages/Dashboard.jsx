...
const Dashboard = () => {
 ...
 <p className="text-slate-500 text-[13px] mt-0.5">Resource allocation & project insights</p>
 </div>
 ...
 <div className="truncate text-xs text-slate-400 ">{project.client}</div>
 </div>
 ),
 },
 {
 ...
 <h3 className="text-sm font-semibold text-slate-800 ">Most Active Autonex Users</h3>
 ...
 <p className="min-w-0 flex-1 truncate text-sm font-medium text-slate-800 ">{u.employee_name || u.user_email}</p>
 ...
 <div className="rounded-2xl border border-slate-200/60 bg-white shadow-[0_1px_3px_rgba(0,0,0,0.04)] ">
 <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100 ">
 <h3 className="text-sm font-semibold text-slate-800 ">Most Active Projects</h3>
 <span className="text-xs text-slate-400">By hours</span>
 </div>
 ...
 >
 <span className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-full text-xs font-bold ${['bg-amber-100 text-amber-700','bg-slate-100 text-slate-600','bg-orange-100 text-orange-700'][idx] || 'bg-slate-50 text-slate-500'}`}>{idx + 1}</span>
 <p className="min-w-0 flex-1 truncate text-sm font-medium text-slate-800 ">{p.name}</p>
