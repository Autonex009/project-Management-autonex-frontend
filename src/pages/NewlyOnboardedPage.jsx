...
const NewlyOnboardedPage = ({ embedded = false }) => {
  ...
  return (
    <div
      className={embedded ? "" : "space-y-8 animate-in fade-in duration-500"}
    >
      {/* Header */}
      {!embedded && (
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
          <div>
            ...
            <p className="text-slate-500 text-sm mt-1">
              Annotation employees not yet assigned to a project, ranked by
              assessment performance. Allocate the best fits straight from here.
            </p>
          </div>
        ...
        <div className="p-4 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div>
            ...
            <p className="text-[11px] text-slate-400 font-medium mt-0.5">
              Click a row to see the per-module breakdown.
            </p>
          </div>
          <SearchBar
            value={search}
        ...
        {filtered.length === 0 ? (
          <div className="p-12 text-center flex flex-col items-center max-w-md mx-auto">
            ...
            <p className="font-bold text-slate-500 text-base">
              No unassigned annotation candidates
            </p>
            <p className="text-sm text-slate-400 text-center mt-1">
              Annotation employees appear here until they're allocated to a
              project. New hires show up automatically.
            </p>
          </div>
        ) : (
                ...
                {filtered.map((c) => (
                  <React.Fragment key={c.userId}>
                    <tr
                      onClick={() =>
                        setExpandedRow(
                          expandedRow === c.userId ? null : c.userId,
                        )
                      }
                    ...
                    >
                      ...
                      <td className="py-4 px-6 text-center">
                        <span
                          className={`inline-flex items-center gap-1 font-extrabold px-3 py-1 rounded-full border text-xs ${scoreBadgeClass(c.overallScore ?? 0)}`}
                        >
      ...
      {allocatingCandidate && (
        <AllocationModalV2
          ...
          excludeProjectIds={(allocatingCandidate.allocations || []).map(
            (a) => a.projectId,
          )}
        />
      )}
