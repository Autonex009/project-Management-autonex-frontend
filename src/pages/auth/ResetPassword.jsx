...
const themeMap = {
  admin: {
    accent: "admin",
    shell:
      "bg-[radial-gradient(circle_at_top,_rgba(37,99,235,0.16),_transparent_30%),linear-gradient(180deg,#020617_0%,#0f172a_100%)]",
    card: "border-white/10 bg-slate-950/75 text-slate-100",
    input:
      "border-slate-800 bg-slate-900/70 text-slate-100 placeholder:text-slate-600 focus:border-blue-500 focus:ring-blue-500/20",
    button:
      "bg-gradient-to-r from-[#103ea8] via-[#1c4fd1] to-[#2b67ff] shadow-[0_18px_40px_rgba(29,78,216,0.35)] hover:brightness-110",
  ...
  pm: {
    accent: "pm",
    shell:
      "bg-[radial-gradient(circle_at_top,_rgba(37,99,235,0.14),_transparent_32%),linear-gradient(180deg,#eff6ff_0%,#f8fafc_100%)]",
    ...
    button:
      "bg-[#123fa9] shadow-[0_16px_38px_rgba(18,63,169,0.26)] hover:bg-[#0f348a]",
  ...
  employee: {
    accent: "employee",
    shell:
      "bg-[radial-gradient(circle_at_top,_rgba(16,185,129,0.14),_transparent_32%),linear-gradient(180deg,#ecfdf5_0%,#f8fafc_100%)]",
    card: "border-slate-200 bg-white/95 text-slate-900",
    input:
      "border-slate-200 bg-white text-slate-900 placeholder:text-slate-400 focus:border-emerald-500 focus:ring-emerald-100",
    button:
      "bg-emerald-600 shadow-[0_16px_38px_rgba(5,150,105,0.24)] hover:bg-emerald-700",
    label: "text-slate-700",
    helper: "text-slate-500",
    border: "border-slate-200",
...
const ResetPassword = () => {
  ...
  const theme = useMemo(
    () => themeMap[searchParams.get("role")] || themeMap.employee,
    [searchParams],
  );

  const resetPasswordMutation = useMutation({
    mutationFn: (nextPassword) =>
    ...
    <div className="min-h-screen flex">
      <AuthBrandPanel
        ...
        highlights={[
          {
            title: "Secure Token",
            copy: "The reset token is validated against the database before any password update.",
          },
          {
            title: "One-Time Use",
            copy: "Successful resets immediately invalidate the stored token.",
          },
        ]}
      />

      <div
        className={`flex flex-1 items-center justify-center p-8 ${theme.shell}`}
      >
        <div
          className={`w-full max-w-md rounded-[28px] border p-8 shadow-[0_25px_80px_rgba(15,23,42,0.12)] backdrop-blur ${theme.card}`}
        >
          <Link
            to={theme.backTo}
          ...
          {isComplete ? (
            <div className="mt-8 text-center">
              ...
              <p className={`mt-2 text-sm ${theme.helper}`}>
                Your password has been changed successfully. You can sign back
                in with it now.
              </p>
              <Link
                to={theme.backTo}
            ...
            <>
              <div className="mt-6">
                ...
                <p className={`mt-2 text-sm ${theme.helper}`}>
                  Set a new password for your account. Slack reset links expire
                  after 15 minutes.
                </p>
              </div>
              ...
              <form onSubmit={handleSubmit} className="mt-8 space-y-5">
                <div>
                  <label
                    className={`mb-1.5 block text-sm font-medium ${theme.label}`}
                  >
                    New Password
                  </label>
                  <div className="relative">
                    ...
                    <input
                      ...
                      className={`w-full rounded-2xl border py-3 pl-11 pr-11 text-sm outline-none transition-all focus:ring-2 ${theme.input}`}
                ...
                {!token && (
                  <div className="rounded-2xl border border-red-200 bg-red-50 p-3 text-sm font-medium text-red-600">
                    This reset link is missing its token. Request a new Slack
                    reset link.
                  </div>
                )}
              ...
              <div className={`mt-6 border-t pt-5 text-sm ${theme.border}`}>
                <p className={theme.helper}>
                  If this link has expired or already been used, request a fresh
