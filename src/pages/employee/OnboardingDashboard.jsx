                    ...
                    {modules.length === 0 ? (
                        ...
                        <ul className="divide-y divide-slate-100">
                            {modules.map((m, idx) => {
                                ...
                                return (
                                    <li key={m.id} className={`p-6 transition-all duration-200 group ${
                                        isLocked ? 'bg-slate-50/30 opacity-75' : 'hover:bg-slate-50/60'
                                    }`}>
                                        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                                            ...
                                            <div className="flex items-center gap-4 flex-1">
                                                ...
                                                <div>
                                                    ...
                                                    <p className={`text-sm line-clamp-1 mt-0.5 max-w-2xl ${
                                            ...
                                            <div className="flex items-center gap-6 md:w-1/3 justify-end">
                                                <div className="hidden md:flex flex-col items-end gap-1 flex-1">
                                                    <div className="flex justify-between w-full max-w-[120px] text-[10px] font-bold text-slate-400 uppercase">
                                                        <span>Progress</span>
                                                        <span className={isLocked ? 'text-slate-400' : 'text-slate-700'}>{m.progress}%</span>
                                                    </div>
                                                    <div className="h-1.5 w-full max-w-[120px] bg-slate-100 rounded-full overflow-hidden">
                                                        <div className={`h-full rounded-full ${isLocked ? 'bg-slate-300' : 'bg-emerald-500'}`} style={{ width: `${m.progress}%` }}></div>
                                                    </div>
                                                </div>
                                                ...
                                                >
                                                    {isLocked ? 'Locked' : isCompleted ? 'Review' : isStarted ? 'Continue' : 'Start'}
                                                    {isLocked ? <Lock className="w-3 h-3" /> : isCompleted ? <ChevronRight className="w-3.5 h-3.5" /> : <Play className="w-3 h-3 fill-current" />}
                                                </button>
                                            </div>
