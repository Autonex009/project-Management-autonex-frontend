...
const EmployeeDashboard = () => {
  ...
  const activeProjects = myProjects.filter(
    (p) => p.project?.project_status === "active",
  );
  const totalHours = activeProjects.reduce(
    (sum, p) => sum + (p.total_daily_hours || 0),
    0,
  );
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        <StatCard
          ...
          tone={currentLeave ? "rose" : "emerald"}
        />
      </div>
            ...
            <div className="flex items-center justify-between p-5 pb-3">
              <div>
                ...
                <p className="text-sm text-slate-400 mt-0.5">
                  Daily sheets you&apos;re assigned to
                </p>
              </div>
            ...
            {allocLoading ? (
              ...
              <div className="p-8 text-center">
                ...
                <p className="text-sm text-slate-400 mt-1">
                  Contact your PM for project assignments.
                </p>
              </div>
            ) : (
                ...
                {activeProjects.map(({ project, ...alloc }) => (
                  ...
                  >
                      ...
                      <div className="flex-1">
                        ...
                        <div className="flex items-center gap-3 mt-2">
                          {(alloc.role_tags || []).map((tag) => (
                            <span
                              key={tag}
                              className="px-2 py-0.5 text-[11px] font-medium bg-blue-50 text-blue-600 rounded-full"
                            >
                              {tag}
                            </span>
                          ))}
                    ...
                    <div className="mt-3 flex items-center gap-2 text-xs text-slate-400">
                      <span>
                        {alloc.active_start_date
                          ? format(parseISO(alloc.active_start_date), "MMM dd")
                          : "—"}
                      </span>
                      <div className="flex-1 h-1.5 bg-slate-100 rounded-full overflow-hidden">
                        <div
                          ...
                          style={{
                            width: `${getProgress(alloc.active_start_date, alloc.active_end_date)}%`,
                          }}
                        />
                      </div>
                      <span>
                        {alloc.active_end_date
                          ? format(parseISO(alloc.active_end_date), "MMM dd")
                          : "—"}
                      </span>
                    </div>
                  </div>
            ...
            {currentLeave ? (
              <div className="p-3 bg-red-50 border border-red-200 rounded-xl mb-3">
                ...
                <p className="text-xs text-red-500 mt-0.5">
                  {format(parseISO(currentLeave.start_date), "MMM dd")} —{" "}
                  {format(parseISO(currentLeave.end_date), "MMM dd")}
                </p>
              </div>
            ) : (
            ...
            <div className="space-y-3">
              {activeProjects.length > 0 ? (
                activeProjects.map(({ project, ...alloc }) => {
                  const pct =
                    totalHours > 0
                      ? Math.round((alloc.total_daily_hours / totalHours) * 100)
                      : 0;
                  return (
                    <div key={alloc.id}>
