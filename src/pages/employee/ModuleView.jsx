...
const ModuleView = () => {
  ...
  const handleAdvance = () => {
    const nextIdx = moduleData.sections.findIndex(
      (s, idx) => idx > activeSectionIndex && !completedSections.has(s.id),
    );
    if (nextIdx !== -1) {
      setLoading(true);
      onboardingApi
  ...
  const startTrackingVideo = (player) => {
    ...
    trackingIntervalRef.current = setInterval(() => {
      ...
      if (currentTime > maxTimeWatchedRef.current + 2) {
        ...
        toast.error(
          "Skipping forward is not allowed. Please watch the full video.",
          { id: "yt-skip" },
        );
      } else {
        maxTimeWatchedRef.current = Math.max(
          maxTimeWatchedRef.current,
          currentTime,
        );
      }
    }, 500);
            ...
            <div className="mt-4 flex flex-col sm:flex-row items-start sm:items-center gap-4 bg-amber-50 border border-amber-100 rounded-2xl p-4 w-full">
              ...
              <div className="flex-1">
                ...
                <p className="text-xs text-amber-700 mt-0.5">
                  Please take the official certification assessment for this
                  module.
                </p>
              </div>
              <a
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="space-y-4">
                      {currentSection.documents.map((doc) => (
                        <div
                          key={doc.id}
                          className="flex flex-col bg-white border border-slate-200/60 rounded-2xl overflow-hidden shadow-sm"
                        >
              ...
              <div className="bg-white rounded-2xl p-6 md:p-8 mb-8 border border-slate-200/60 shadow-sm animate-in slide-in-from-right-4 duration-300">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-400">
                        Passing score required: {currentPassingScore}%
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-8">
                  {quizSubmitted[currentSection.id] ? (
                    <div
                      className={`${quizSubmitted[currentSection.id].passed ? "bg-emerald-600" : "bg-amber-600"} rounded-2xl p-6 text-center shadow-md text-white`}
                    >
                      ...
                      <p className="text-white/80 text-sm mb-3">
                        ...
                        <span className="font-bold text-white text-lg">
                          {quizSubmitted[currentSection.id].score}%
                        </span>
                        .
                      </p>
                      <div className="inline-flex items-center justify-center p-3 bg-white/10 rounded-xl mb-4 text-xs font-bold uppercase">
                        {quizSubmitted[currentSection.id].correctCount} of{" "}
                        {quizSubmitted[currentSection.id].totalQuestions}{" "}
                        answers correct
                      </div>
                      {!quizSubmitted[currentSection.id].passed && (
                        <div className="mt-2">
            ...
            <footer className="flex justify-between items-center pt-6 border-t border-slate-200 mt-4">
              ...
              (!currentSection.questions ||
                ...
                <Button
                  ...
                  disabled={
                    answeredCurrentQuestions < currentSection.questions.length
                  }
              ...
              {completedSections.has(currentSection.id) && (
                <Button
                  ...
                  className="px-6 py-3 text-xs uppercase tracking-wider font-bold shadow-md hover:shadow-lg"
                >
                  <span>
                    {activeSectionIndex === moduleData.sections.length - 1
                      ? "Finish Module"
                      : "Next Lesson"}
                  </span>
          ...
          {moduleData.sections.map((s, idx) => {
            ...
            return (
              <div key={s.id} className="relative pl-8 pb-8">
                <div
                  className={`absolute left-0 top-1.5 w-6 h-6 rounded-full flex items-center justify-center border z-10 text-[10px] font-bold ${
                    isComplete
                ...
                <p
                  className={`text-[10px] font-bold uppercase ${isComplete || isCurrent ? "text-emerald-600" : "text-slate-400"}`}
                >
                  Section {idx + 1}
                </p>
                <h5
                  className={`text-xs font-bold mt-0.5 ${isCurrent ? "text-slate-900 font-extrabold" : "text-slate-600"}`}
                >
                  {s.title}
                </h5>
              </div>
        ...
        <div className="mt-8 pt-6 border-t border-slate-100">
          <div className="bg-slate-50 rounded-xl p-4 space-y-3">
            ...
            <p className="text-[10px] text-slate-400 leading-relaxed">
              Complete all lessons to clear this module and qualify for the
              final assessment.
            </p>
          </div>
        </div>
          ...
          <Modal.Body className="!p-8 text-center flex flex-col items-center">
            ...
            <div
              className={`w-full rounded-2xl p-6 mb-6 ${
                modalData.passed
                  ? "bg-emerald-50/50 border border-emerald-100/30"
                  : "bg-amber-50/50 border border-amber-100/30"
              }`}
            >
            ...
            <div className="w-full space-y-2">
              {modalData.passed ? (
                <Button
                  ...
                  className="w-full justify-center py-3.5 text-sm uppercase tracking-wide font-bold rounded-2xl shadow-md"
                >
                  {activeSectionIndex === moduleData.sections.length - 1
                    ? "Finish Module"
                    : "Next Lesson"}
