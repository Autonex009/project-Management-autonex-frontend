...
import { ClipboardList, Search, CheckCircle2, Lock, Gift, UserCog, Star, ChevronDown } from 'lucide-react';
...
const AdminPerformancePage = () => {
 ...
 const bonusColumns = [
 {
 key: 'employee', label: 'Employee', width: 'w-[20%]',
 render: (_, ev) => (
 <div className="flex min-w-0 items-center gap-2.5">
 ...
 { key: 'project', label: 'Project', width: 'w-[16%]', render: (_, ev) => <span className="block truncate text-slate-600">{projName(ev.project_id)}</span> },
 {
 ...
 {
 key: 'rating', label: 'Rating', align: 'right', width: 'w-[16%]',
 render: (_, ev) => (
 ...
 <ChevronDown className={`ml-auto h-4 w-4 text-amber-600 transition-transform ${bonusOpen ? 'rotate-180' : ''}`} />
 </button>
 {bonusOpen && (
 <div className="px-3 pb-3">
 <Table variant="untitled" allowOverflow columns={bonusColumns} data={bonusEvals} getRowId={(row) => row.id} />
 </div>
 )}
 </section>
 )}
