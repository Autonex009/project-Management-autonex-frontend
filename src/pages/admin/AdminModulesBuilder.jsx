  ...
  const handleSave = async () => {
    ...
    for (let i = 0; i < sections.length; i++) {
      ...
      for (let j = 0; j < questions.length; j++) {
        ...
        if (!q.question.trim()) {
          toast.error(
            `Section #${i + 1}, Question ${j + 1}: question text is required.`,
          );
          return;
        }
        if ((q.options || []).some((opt) => !opt.trim())) {
          toast.error(
            `Section #${i + 1}, Question ${j + 1}: all four options are required.`,
          );
          return;
        }
      }
    ...
    try {
      ...
      toast.error(
        "Error saving module. Please make sure the backend is running.",
      );
    } finally {
      setIsSaving(false);
    }
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-slate-500">
              {editId
                ? "Update this module's content and settings."
                : "Build interactive onboarding courses with real content."}
            </p>
          </div>
          <Button
            size="lg"
          ...
          >
            {!isSaving && (
              <>
                ...
                {editId ? "Update Module" : "Save Module"}
              </>
            )}
          </Button>
        </div>
          ...
          <div className="space-y-4">
            ...
            <div>
              <label className="text-sm font-bold text-slate-700 mb-1 flex items-center gap-1">
                <Link2 className="h-4 w-4 text-indigo-500" /> Assessment Link
                (Optional)
              </label>
              <input
                type="url"
              ...
              <p className="text-xs text-slate-400 mt-1">
                Candidates will see a "Take Assessment" button that opens this
                link.
              </p>
            </div>
        ...
        <div className="space-y-4">
          {sections.length === 0 && (
            <div className="text-center py-8 text-slate-400 italic border-2 border-dashed border-slate-200 rounded-2xl">
              No sections yet. Click "Add Section" below to start building your
              module.
            </div>
          )}
