...
import { Plus, Edit, Trash2, X, User, ChevronDown, CheckCircle, AlertCircle, Clock, ArrowUpCircle, RotateCcw, MoreVertical, Users, UserCheck, Briefcase, Award, ShieldCheck, Filter, ArrowUpDown, ArrowUp, ArrowDown, Search, Check } from 'lucide-react';
...
const DesignationMultiSelect = ({ options, value, onChange }) => {
 ...
 >
 <span className={`truncate ${value.length === 0 ? 'text-slate-500' : 'text-slate-800 font-medium'}`}>
 {value.length === 0 ? 'Designation' : value.length === 1 ? value[0] : `${value.length} selected`}
 </span>
...
const FilterButton = ({ predefinedSkills, skillFilter, setSkillFilter, designationOptions, designationFilter, setDesignationFilter, typeFilter, setTypeFilter, typeOptions, workModelFilter, setWorkModelFilter, workModelOptions, onChange }) => {
 ...
 const activeCount = (skillFilter ? 1 : 0) + (designationFilter.length > 0 ? 1 : 0) + (typeFilter ? 1 : 0) + (workModelFilter ? 1 : 0);
 ...
 <label className="text-[10px] font-semibold uppercase tracking-wide text-slate-400">Skills</label>
 <Dropdown
 options={[{ value: '', label: 'Skills' }, ...predefinedSkills.map((s) => ({ value: s, label: s }))]}
 value={skillFilter}
 ...
 <div className="space-y-1">
 <label className="text-[10px] font-semibold uppercase tracking-wide text-slate-400">Designation</label>
 ...
 <Dropdown
 options={[{ value: '', label: 'Types' }, ...typeOptions.map((t) => ({ value: t, label: t }))]}
 value={typeFilter}
 onChange={(val) => { setTypeFilter(val); onChange?.(); }}
 placeholder="Types"
 optionsClassName="w-full"
 />
 </div>
 <div className="space-y-1">
 <label className="text-[10px] font-semibold uppercase tracking-wide text-slate-400">Work Model</label>
 <Dropdown
 options={[{ value: '', label: 'Work Models' }, ...workModelOptions.map((w) => ({ value: w, label: w }))]}
 value={workModelFilter}
 onChange={(val) => { setWorkModelFilter(val); onChange?.(); }}
 ...
 <button
 type="button"
 onClick={() => { setSkillFilter(''); setDesignationFilter([]); setTypeFilter(''); setWorkModelFilter(''); onChange?.(); }}
...
const EmployeesPage = () => {
 ...
 const childProjects = subProjects.filter((sp) => String(sp.main_project_id) === String(mp.id));
 ...
 childProjects.forEach((sp) => { if (sp.name) employeeProjectsMap[pmId].add(sp.name); });
 });
 });
 ...
 const designationValues = [...new Set(employees.map((e) => e.designation).filter(Boolean))].sort();
 const typeValues = [...new Set(employees.map((e) => e.employee_type).filter(Boolean))].sort();
 ...
 const cycleNameSort = () => setSortBy((s) => (s === 'name-asc' ? 'name-desc' : s === 'name-desc' ? '' : 'name-asc'));
 ...
 const matchesColDesignation = !colDesignation || employee.designation === colDesignation;
 const matchesColType = !colType || employee.employee_type === colType;
 const matchesColWorkModel = !colWorkModel || (employee.work_model || 'WFO') === colWorkModel;
 ...
 return matchesSearch && matchesSkill && matchesDesignation && matchesColDesignation && matchesColType && matchesColWorkModel && matchesIdle && matchesStatus;
 });
 ...
 useEffect(() => {
 setCurrentPage(1);
 }, [searchQuery, skillFilter, designationFilter, idleOnly, statusParam, sortBy, colDesignation, colType, colWorkModel]);
 ...
 <p className="text-slate-500 text-[13px] mt-0.5">Manage team members and their availability</p>
 </div>
 </div>
 </div>
 ...
 <span className="font-normal text-purple-600 ">{hybridCount}</span>
 </div>
 </div>
 </div>
 ...
 <span className="font-normal text-emerald-600 ">{activeCount}</span>
 </div>
 <div className="flex items-center gap-1">
 <span className="w-1.5 h-1.5 rounded-full bg-slate-400" />
 <span className="text-slate-500 font-normal">Inactive:</span>
 <span className="font-normal text-slate-500 ">{inactiveCount}</span>
 </div>
 <div className="flex items-center gap-1">
 ...
 <span className="font-normal text-emerald-600 ">{fullTimeCount}</span>
 </div>
 <div className="flex items-center gap-1">
 <span className="w-1.5 h-1.5 rounded-full bg-amber-500" />
 <span className="text-slate-500 font-normal">Intern:</span>
 <span className="font-normal text-amber-600 ">{internCount}</span>
 </div>
 <div className="flex items-center gap-1">
 <span className="w-1.5 h-1.5 rounded-full bg-sky-500" />
 <span className="text-slate-500 font-normal">Contract:</span>
 <span className="font-normal text-sky-500 ">{contractCount}</span>
 </div>
 </div>
 </div>
 ...
 <span className="font-normal text-slate-700 ">{annotatorCount}</span>
 </div>
 <div className="flex items-center gap-1">
 <span className="w-1.5 h-1.5 rounded-full bg-purple-500" />
 ...
 key: 'name',
 label: (
 <button type="button" onClick={cycleNameSort} className="inline-flex items-center gap-1 hover:text-slate-900" title="Sort by name">
 Employee
 {sortBy === 'name-asc' ? <ArrowUp className="w-3.5 h-3.5 text-indigo-600" /> : sortBy === 'name-desc' ? <ArrowDown className="w-3.5 h-3.5 text-indigo-600" /> : <ArrowUpDown className="w-3.5 h-3.5 text-slate-400" />}
 </button>
 ),
 width: 'w-[19%]',
 render: (value, row) => {
 ...
 <div className={`absolute left-0 ${positionClass} hidden group-hover:block z-40 p-2.5 bg-white rounded-xl shadow-xl border border-slate-200 min-w-[180px] max-w-[280px] pointer-events-none`}>
 <div className="text-[13px] font-semibold text-slate-800 break-words">{value}</div>
 <div className="text-[12px] text-slate-500 break-words mt-0.5">{row.email}</div>
 </div>
 </div>
 </div>
 );
 ...
 Reporting Manager{managers.length > 1 ? 's' : ''} ({managers.length})
 </div>
 ...
 <div className="text-xs font-semibold text-slate-400 uppercase tracking-wider">
 Assigned Project{list.length > 1 ? 's' : ''} ({list.length})
 </div>
 <div className="flex flex-wrap gap-1">
 {list.map((name, idx) => (
 <span
