...
const EmployeeKPIPanel = ({
  ...
  const employeeStats = useMemo(() => {
    return employees.map((emp) => {
      ...
      if (selectedMonth !== "all" && monthBoundary) {
        const { startDate: mStart, endDate: mEnd } = monthBoundary;
        filteredWfh = empWfh.filter(
          (w) => w.wfh_date >= mStart && w.wfh_date <= mEnd,
        );
      }

      const approvedWfhCount = filteredWfh.filter(
        (w) => w.status === "approved",
      ).length;
      const pendingWfhCount = filteredWfh.filter(
        (w) => w.status === "pending",
      ).length;
      ...
      let reliabilityScore =
        100 - approvedLeaveDays * 5 - approvedFlaggedCount * 15;
      reliabilityScore = Math.max(
        0,
        Math.min(100, Math.round(reliabilityScore)),
      );
      ...
      let performanceAdvice =
        "Outstanding attendance. Perfect availability for critical deliverable paths.";
      ...
      if (reliabilityScore < 50) {
        ...
        performanceAdvice =
          "High absenteeism has significantly impacted attendance. Review schedule alignment and check for policy compliance.";
        ...
        performanceAdvice =
          "Moderate leave count. Ensure project milestones have clear handovers and backups assigned.";
      } else if (reliabilityScore < 90) {
        rating = "Good Presence";
        ratingColor = "text-blue-700 bg-blue-50 border-blue-200";
        ratingDot = "bg-blue-500";
        performanceAdvice =
          "Healthy work attendance. Leave utilization is within expected parameters.";
      } else if (approvedLeaveDays > 0) {
        performanceAdvice =
          "Strong attendance record. Minimal leave interruptions.";
      }
  ...
  const overallStats = useMemo(() => {
    ...
    const totalLeaves = employeeStats.reduce(
      (acc, s) => acc + s.approvedLeaveDays,
      0,
    );
    const totalWfh = employeeStats.reduce(
      (acc, s) => acc + s.approvedWfhCount,
      0,
    );
    const avgLeaves = (totalLeaves / employeeStats.length).toFixed(1);
    const highAbsenteeism = employeeStats.filter(
      (s) => s.reliabilityScore < 75,
    ).length;
  ...
  const sortedAndFilteredStats = useMemo(() => {
    return employeeStats
      .filter((item) => {
        ...
        const matchesSearch =
          name.includes(q) || designation.includes(q) || dept.includes(q);
  ...
  const renderSortIcon = (field) => {
    if (sortBy !== field)
      return <ArrowUpDown className="w-3.5 h-3.5 ml-1 opacity-45 shrink-0" />;
    return sortOrder === "asc" ? (
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-white rounded-2xl border border-slate-200/60 p-5 shadow-sm hover:shadow-md transition-shadow">
          ...
          <p className="text-2xl font-bold text-slate-800">
            {overallStats.highAbsenteeism} Members
          </p>
          <p className="text-xs text-slate-400 mt-1">
            High Leave Utilization (&lt;75% presence)
          </p>
        </div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <h2 className="text-lg font-bold text-slate-800">
          Employee Presence & Reliability Scorecard
        </h2>
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-6">
                  ...
                  <div className="bg-gradient-to-r from-indigo-50 to-slate-50 border border-indigo-100/60 rounded-2xl p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div className="space-y-1 bg-transparent">
                      <p className="text-xs font-semibold text-indigo-500 uppercase tracking-wider">
                        Attendance Profile & Performance Impact
                      </p>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="border border-slate-100 rounded-xl p-4 bg-white shadow-sm">
                      ...
                      <p className="text-3xl font-extrabold text-slate-800 mt-2 font-mono">
                        {approvedLeaveDays} Days
                      </p>
                    ...
                    <div className="border border-slate-100 rounded-xl p-4 bg-white shadow-sm">
                      ...
                      <p className="text-3xl font-extrabold text-slate-800 mt-2 font-mono">
                        {approvedWfhCount} Days
                      </p>
                      <p className="text-xs text-slate-400 mt-3 border-t border-slate-50 pt-2.5">
                        Approved requests: {wfhList.length}
                      </p>
                    </div>
                  </div>
                    ...
                    <div className="border border-slate-100 rounded-xl p-4 space-y-4 bg-white">
                      {["paid", "casual_sick", "floater"].map((type) => {
                        ...
                        const isEmpIntern =
                          ...
                          isIntern(employee.employee_type);
                        const maxDays =
                          selectedMonth === "all"
                            ? type === "paid"
                              ? 12
                        ...
                        const percentage =
                          maxDays > 0
                            ? Math.min(100, (count / maxDays) * 100)
                            : 0;
                        return (
                    ...
                    <div className="border border-slate-100 rounded-xl overflow-hidden shadow-sm bg-white">
                      {leaveList.length === 0 ? (
                        <p className="p-8 text-center text-sm text-slate-400">
                          No leaves logged in this period.
                        </p>
                      ) : (
                        <div className="divide-y divide-slate-100">
                          {leaveList.map((leave) => {
                            ...
                            let days = 0;
                            if (selectedMonth === "all") {
                              days = getWorkingDayCount(
                                leave.start_date,
                                leave.end_date,
                                leave.is_half_day,
                              );
                            } else if (monthBoundary) {
                              const { startDate: mStart, endDate: mEnd } =
                                monthBoundary;
                              const overlapStart =
                                leave.start_date < mStart
                                  ? mStart
                                  : leave.start_date;
                              const overlapEnd =
                                leave.end_date > mEnd ? mEnd : leave.end_date;
                              days = getWorkingDayCount(
                                overlapStart,
                                overlapEnd,
                                leave.is_half_day,
                            ...
                            return (
                              ...
                              >
                                ...
                                <div className="flex items-center gap-3 shrink-0">
                                  <span className="font-mono font-semibold text-sm text-slate-700 bg-slate-100 rounded px-2 py-0.5">
                                    {days}d{" "}
                                    {selectedMonth !== "all" &&
                                      `(of ${getWorkingDayCount(leave.start_date, leave.end_date, leave.is_half_day)}d)`}
                                  </span>
                  ...
                  <div className="space-y-3 bg-transparent">
                    ...
                    <div className="border border-slate-100 rounded-xl overflow-hidden shadow-sm bg-white">
                      {wfhList.length === 0 ? (
                        <p className="p-8 text-center text-sm text-slate-400">
                          No WFH requests logged in this period.
                        </p>
                      ) : (
                        <div className="divide-y divide-slate-100">
                          {wfhList.map((wfh) => (
                            ...
                            >
                              <div>
                                <p className="font-semibold text-slate-800 text-sm">
                                  {format(
                                    new Date(wfh.wfh_date + "T00:00:00"),
                                    "MMM d, yyyy",
                                  )}
                                </p>
                                {wfh.reason && (
                                  <p className="text-xs text-slate-500 mt-1 italic">
                                    &ldquo;{wfh.reason}&rdquo;
                                  </p>
                                )}
                              </div>
                              <div className="shrink-0">
                                {wfh.status === "approved" ? (
                                  ...
                                  <span className="text-xs font-semibold text-amber-600 bg-amber-50 border border-amber-100 rounded px-2 py-0.5">
                                    Pending
                                  </span>
                                ) : (
                                  <span className="text-xs font-semibold text-red-600 bg-red-50 border border-red-100 rounded px-2 py-0.5">
                                    Rejected
                                  </span>
                                )}
                              </div>
