...
const AllocationModalV2 = ({
  ...
  useEffect(() => {
    ...
    setCapacityError(false);
    recommendationsApi
      .getByProject(formData.project_id)
      .then((data) => {
  ...
  useEffect(() => {
    if (editingAllocation) {
      setFormData({
        ...
        active_start_date:
          editingAllocation.active_start_date ||
  ...
  const validateForm = () => {
    ...
    if (formData.role_tags.length > 0) {
      const totalDistributed = Object.values(formData.time_distribution).reduce(
        (sum, h) => sum + h,
        0,
      );
      if (totalDistributed !== formData.total_daily_hours) {
        errors.push(
          `Time distribution (${totalDistributed}h) must equal total daily hours (${formData.total_daily_hours}h)`,
        );
      }
    }
    ...
    if (
      ...
      formData.active_end_date < formData.active_start_date
    ) {
                ...
                options={projects
                  .filter(
                    (p) =>
                      ...
                      !excludeProjectIds.map(String).includes(String(p.id)),
                  )
                  .map((proj) => ({
          ...
          {projectInsights &&
            ...
            (() => {
              ...
              const allocated = allocations.filter(
                (a) => String(a.sub_project_id) === String(selectedProject.id),
              ).length;
              const remaining = required - allocated;
              const req = Array.isArray(selectedProject.required_expertise)
                ? selectedProject.required_expertise
                : [];
              const have = (candidateSkills || []).map((s) =>
                String(s).trim().toLowerCase(),
              );
              const matched = req.filter((s) =>
                have.includes(String(s).trim().toLowerCase()),
              ).length;
                ...
                <div className="rounded-lg border border-slate-200 bg-slate-50/60 p-4 space-y-3">
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <div className="flex items-center gap-1.5 shrink-0">
                      <span
                        className={`px-1.5 py-0.5 text-[10px] font-bold uppercase rounded border ${selectedProject.is_annotation ? "bg-indigo-50 text-indigo-600 border-indigo-100" : "bg-slate-100 text-slate-500 border-slate-200"}`}
                      >
                        {selectedProject.is_annotation
                          ? "Annotation"
                          : "Non-annotation"}
                      </span>
                      {selectedProject.priority && (
                        <span className="px-1.5 py-0.5 text-[10px] font-bold uppercase rounded border bg-white text-slate-600 border-slate-200">
                          {selectedProject.priority}
                        </span>
                      )}
                    </div>
                  ...
                  <div>
                    <div className="flex items-center justify-between text-xs mb-1">
                      <span className="flex items-center gap-1 text-slate-500 font-medium">
                        <Users className="w-3.5 h-3.5" /> Manpower
                      </span>
                      <span className="font-bold text-slate-700">
                        {allocated}
                        {required ? ` / ${required}` : ""} allocated
                        {required
                          ? remaining > 0
                    ...
                    {required > 0 && (
                      <div className="h-1.5 w-full bg-slate-200 rounded-full overflow-hidden">
                        <div
                          className={`h-full rounded-full ${allocated >= required ? "bg-emerald-500" : "bg-indigo-500"}`}
                          style={{
                            width: `${Math.min(100, (allocated / required) * 100)}%`,
                          }}
                        />
                      </div>
                    )}
                  ...
                  <div>
                    ...
                    {req.length === 0 ? (
                      ...
                      <div className="flex flex-wrap gap-1">
                        {req.map((s) => {
                          const ok = have.includes(
                            String(s).trim().toLowerCase(),
                          );
                          return (
                            <span
                              key={s}
                              className={`px-1.5 py-0.5 text-[10px] font-semibold rounded border ${ok ? "bg-emerald-50 text-emerald-700 border-emerald-200" : "bg-slate-100 text-slate-400 border-slate-200"}`}
                            >
                              {ok ? "✓ " : ""}
                              {s}
                            </span>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-[11px] text-slate-600">
                    <div className="flex items-center gap-1">
                      ...
                      {selectedProject.end_date || "open-ended"}
                    </div>
                    <div className="flex items-center gap-1">
                      <Target className="w-3.5 h-3.5 text-slate-400 shrink-0" />{" "}
                      {selectedProject.daily_target || 0}/day ·{" "}
                      {selectedProject.total_tasks || 0} tasks
                    </div>
                  </div>
                  ...
                  <div className="pt-2 border-t border-slate-200/70 flex items-center justify-between text-xs">
                    <span className="flex items-center gap-1 text-slate-500 font-medium">
                      <Activity className="w-3.5 h-3.5" /> Capacity
                    </span>
                    {capacityLoading ? (
                      ...
                      <span className="flex items-center gap-2">
                        <span
                          className={`px-2 py-0.5 text-[10px] font-bold uppercase rounded-full border ${cap.cls}`}
                        >
                          {cap.label}
                        </span>
                        <span className="text-[11px] text-slate-500">
                          team {capacity.team_size ?? 0} ·{" "}
                          {Math.round(
                            capacity.avg_daily_hours_per_employee ?? 0,
                          )}
                          h/day
                        </span>
                      </span>
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setFormData((prev) => ({
                  ...prev,
                  total_daily_hours: parseInt(e.target.value) || 8,
                }))
              }
              className="w-32 px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
            />
            <p className="mt-1 text-xs text-gray-500">
              Hours per day for this allocation
            </p>
          </div>
            ...
            <div className="border border-gray-200 rounded-lg p-4">
              ...
              {formData.role_tags.length > 0 && (
                <div className="space-y-3">
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2">
                        <input
                          ...
                          onChange={(e) =>
                            handleDistributionChange(role, e.target.value)
                          }
                  ...
                  {/* Balance Indicator */}
                  <div
                    className={`p-3 rounded-md ${isBalanced ? "bg-green-50 border border-green-200" : "bg-yellow-50 border border-yellow-200"}`}
                  >
                    <div className="flex items-center justify-between text-sm">
                      ...
                      >
                        {totalDistributed} / {formData.total_daily_hours} hours
                        allocated
                      </span>
                    </div>
                  </div>
              ...
              <label className="flex items-center gap-2 mb-3">
                <input
                  ...
                  onChange={(e) =>
                    setFormData((prev) => ({
                      ...prev,
                      override_flag: e.target.checked,
                    }))
                  }
