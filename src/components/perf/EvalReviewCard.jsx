...
const EvalReviewCard = ({ evaluation, personName, reviewerId }) => {
  ...
  const invalidate = () =>
    queryClient.invalidateQueries({ queryKey: ["perf-evals"] });
  ...
  const submitReview = () => {
    ...
    if (missingFeedback) {
      toast.error(
        `Add feedback for the rejected parameter “${missingFeedback.name}”`,
      );
      return;
    }
        ...
        {!editing && (
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              title="Delete"
              className="inline-flex items-center gap-1 rounded-lg border border-slate-200 px-2 py-1 text-xs font-medium text-slate-500 hover:bg-rose-50 hover:text-rose-600"
            >
            ...
            >
              <div className="flex flex-wrap items-center justify-between gap-x-4 gap-y-1.5">
                ...
                <div className="flex items-center gap-2.5">
                  ...
                  <StarRating
                    value={r.pm_rating || null}
                    onChange={(v) => setRow(r.name, { pm_rating: v })}
                    showLabel={false}
                    size="text-base"
                  />
                  ...
                  <div className="flex items-center gap-3 text-xs">
                    <label className="inline-flex cursor-pointer items-center gap-1.5">
                      <input
                        type="radio"
                        name={`decision-${evaluation.id}-${r.name}`}
                        checked={r.approved}
                        onChange={() => setRow(r.name, { approved: true })}
                    ...
                    <label className="inline-flex cursor-pointer items-center gap-1.5">
                      <input
                        ...
                        name={`decision-${evaluation.id}-${r.name}`}
                        checked={!r.approved}
                        onChange={() => setRow(r.name, { approved: false })}
          ...
          <div className="rounded-lg border border-amber-200 bg-amber-50/60 px-3 py-2.5">
            <label className="flex items-center justify-between gap-3">
              <span className="inline-flex items-center gap-2 text-sm font-medium text-amber-800">
                <Gift className="h-4 w-4" /> Suggest for a bonus
              </span>
              <button
                type="button"
                role="switch"
              ...
              >
                <span
                  className={`inline-block h-5 w-5 transform rounded-full bg-white shadow transition-transform ${bonus ? "translate-x-5" : "translate-x-0.5"}`}
                />
              </button>
            </label>
            {bonus && (
