...
const Breadcrumbs = ({
  ...
  const parentCrumb = hasParent ? items[items.length - 2] : null;

  const handleBack = () => {
    if (parentCrumb?.path) {
      navigate(parentCrumb.path);
    } else if (items.length === 1 && homeHref) {
      navigate(homeHref);
    } else {
      navigate(-1);
    }
    ...
    >
      {showBackButton && hasParent && (
        <button
          type="button"
          onClick={handleBack}
          title={`Back to ${parentCrumb?.name || "previous page"}`}
          className="flex items-center justify-center p-1 -ml-1 text-slate-500 hover:text-slate-800 hover:bg-slate-100 rounded-md transition-colors cursor-pointer mr-0.5"
        >
