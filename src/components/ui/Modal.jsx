...
function ModalHeader({ children, onClose, className = "" }) {
  return (
    <div
      className={`flex items-start justify-between px-6 py-5 border-b border-slate-100 flex-shrink-0 ${className}`}
    >
...
function ModalFooter({ children, align = "end", className = "" }) {
  return (
    <div
      className={`flex items-center gap-3 px-6 py-4 border-t border-slate-100 bg-white flex-shrink-0 ${FOOTER_ALIGN[align] || FOOTER_ALIGN.end} ${className}`}
    >
      {children}
    </div>
  );
...
function CompactModalFooter({ children, align = "end", className = "" }) {
  return (
    <div
      className={`
