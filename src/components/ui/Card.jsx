...
export const Card = ({ children, className = '', loading = false, onClick }) => (
 <div
 onClick={onClick}
 className={`bg-white rounded-2xl border border-slate-200/60 shadow-[0_1px_3px_rgba(0,0,0,0.04)] hover:shadow-[0_4px_12px_rgba(0,0,0,0.06)] transition-all duration-300 ${onClick ? 'cursor-pointer hover:border-slate-300/80 ' : ''} ${className}`}
 >
...
export const CardHeader = ({ title, subtitle, action, badge }) => (
 ...
 {badge && <span className="px-2 py-0.5 text-xs font-medium bg-blue-50 text-blue-600 rounded-full">{badge}</span>}
 </div>
