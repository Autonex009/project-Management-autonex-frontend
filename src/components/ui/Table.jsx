...
export const ColumnTemplates = {
  ...
  avatarInfo: (key, label, subtitleKey, opts = {}) => ({
    ...
    render: (value, row) => (
      <div className="flex items-center gap-3">
        <div
          className={`w-10 h-10 rounded-lg flex items-center justify-center flex-shrink-0 text-white text-sm font-bold ${opts.avatarClass || "bg-gradient-to-br from-blue-500 to-purple-600"}`}
        >
  ...
  badge: (key, label, colorMap = {}) => ({
    ...
    render: (value) => {
      ...
      return value ? (
        <span
          className={`inline-flex px-2.5 py-0.5 rounded-full text-xs font-medium border ${colorClass}`}
        >
          {value}
        </span>
      ) : (
      ...
      return (
        <div className="flex flex-wrap gap-1">
          {items.map((tag, i) => (
            <span
              key={i}
              className="inline-flex px-2 py-0.5 rounded-md text-xs font-medium bg-slate-100 text-slate-600 border border-slate-200"
            >
              {tag}
            </span>
          ))}
