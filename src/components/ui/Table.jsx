...
export const Table = ({
 ...
 pageSize = 10,
 onPageChange,
 totalItems = null, // when set, pagination is server-side: `data` is the current page only
 onEdit,
 onDelete,
 loading = false,
 skeletonRows = 5,
 ...
 // Server-side: `data` already holds only the current page, so render it as-is.
 const paginatedData = serverSide ? (data || []) : (data?.slice(startIndex, startIndex + pageSize) || []);
 ...
 const headCase = airy ? 'normal-case tracking-normal text-slate-600 ' : 'uppercase tracking-wider text-slate-700 ';
 const headWeight = airy ? 'font-semibold' : 'font-bold';
 const theadBg = airy ? 'bg-slate-50 ' : 'bg-slate-50/80 ';
 ...
 {count != null && (
 <span className="rounded-full border border-slate-200 px-2 py-0.5 text-[11px] font-medium text-slate-500 ">{count}</span>
 )}
 </div>
 {headerAction}
 </div>
 ...
 <thead className={`${theadBg} ${allowOverflow && !title ? 'rounded-t-2xl' : ''} border-b border-slate-100 `}>
 <tr>
