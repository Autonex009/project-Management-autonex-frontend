...
import { UserPlus, Users, Mail, ArrowRight, Home, Building2 } from 'lucide-react';
...
const AllocationPopover = ({
 ...
 const location = emp ? locationByEmployeeId?.get?.(emp.id) : undefined;
 return (
 <li
 key={alloc.id}
 ...
 <span className="inline-flex items-center gap-1 rounded-full bg-violet-50 px-1.5 py-0.5 text-[10px] font-semibold text-violet-700 shrink-0" title="Working from home today">
 <Home className="w-3 h-3" />
 WFH
 </span>
 ) : location === 'WFO' ? (
 <span className="inline-flex items-center gap-1 rounded-full bg-slate-100 px-1.5 py-0.5 text-[10px] font-semibold text-slate-600 shrink-0" title="Working from office today">
